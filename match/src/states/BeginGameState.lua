...
function BeginGameState:enter(def)
    ...
    -- spawn a board and place it towards the right side of the screen
    self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level)     -- pass it the level so that it knows which level it has to follow

    -- while not self.board:anyMatches() do
    --     self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level)
    -- end 

    -- ensure that there are possible matches
    -- while not self.board:anyMatches() do
    --     self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level)     
    -- end
