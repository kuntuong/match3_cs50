    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- self.swapTiles(self.hightlightTile, self.board.tile[y][x], true)
...
function PlayState:calculateMatches(calculatedMatches)
    ...
    local matches = self.board:calculateMatches() or calculatedMatches
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for j, tile in pairs(match) do
                -- add a bit more score if there is a symbol on it
        ...
        --     Timer.tween(0.5, {
        --         [self] = {noMatchLabelY = VIRTUAL_HEIGHT / 2 - 6}
        --     })

        --     :finish(function()
        --         Timer.after(2, function()
        --             Timer.tween(0.5, {
        --                 [self] = {noMatchLabelY = VIRTUAL_HEIGHT + 38}
        --             })
...
function PlayState:render()
    ...
    if self.noMatches then
        ...
        love.graphics.rectangle('fill', 0, self.noMatchesLabelY - 8, VIRTUAL_WIDTH, 32)
        love.graphics.setColor(255, 255, 255, 255)
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('No matches found! Refreshing board',
            0, self.noMatchesLabelY, VIRTUAL_WIDTH, 'center')
    end
end